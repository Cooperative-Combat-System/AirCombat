    ...
    def run(self):
        ...
        for episode in range(episodes):
            ...
            for step in range(self.buffer_size):
                ...
                # ------------------ NEW: 记录 reward 分量到 wandb ------------------
                # infos 可能是 np.ndarray(object) 或 list[dict]；统一成可迭代的 list
                if isinstance(infos, np.ndarray):
                    infos_iter = infos.tolist()
                else:
                    infos_iter = infos

                # 收集 rc/*（每步均值分量）与 erc/*（回合统计）
                step_logs = {}
                ep_logs = {}
                ...
                # 用“全局步数”作为 wandb 的 step，使曲线横轴对齐
                # 注意：此时 self.total_num_steps 还没更新，自己算一个“当前步”
                cur_global_step = (
                        episode * self.buffer_size * self.n_rollout_threads
                        + (step + 1) * self.n_rollout_threads
                )

                if self.all_args.use_wandb:
                    if step_logs:
                        wandb.log(step_logs, step=cur_global_step)
                    if ep_logs:
                        # 有时一个 rollout 内可能结束多个并行 episode；这里聚合为一条日志
                        wandb.log(ep_logs, step=cur_global_step)
                # ------------------ NEW END ------------------
